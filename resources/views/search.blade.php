...
@php
$q = request('q', '');
$people = \App\Models\User::where('first_name', 'like', "%$q%")
            ...
            <div class="flex items-center justify-between gap-4 rounded-2xl border border-app bg-white p-4">
              ...
              <div class="flex items-center gap-4 min-w-0">
                <img
                  ...
                  src="{{ $p->profile_pic ?? 'https://i.pravatar.cc/120?img=' . $p->user_id }}"
                  alt="avatar">

                <div class="leading-tight min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    <div class="font-extrabold text-app truncate">{{ $p->first_name }} {{ $p->last_name }}</div>
                  </div>

                  <div class="text-sm text-app-muted">@{{ $p->username }}</div>
